...
function record(baseTaskId: string, comparisonArm: NyxBiologicalConnectomeAblationArm,
  ...
  const connected = comparisonArm === "NYX_CONNECTOME" || profiled;
  return { baseTaskId, comparisonArm, frozenTaskContentDigest: `digest-${baseTaskId}`,
    ...
    connectomeTraces: connected ? [{ additionalModelCalls: 0, grantsAuthority: false,
      architecture: { profileDigest: profiled
        ? NYX_BIOLOGICAL_CONNECTOME_ABLATION.treatmentProfileDigest : null } }] : [], ...overrides };
}
...
const measurable = assess(population((task, arm) => arm === "NYX_BIOLOGICAL_CONNECTOME" || task === "TASK-3"));
check(measurable.decision === "MEASURABLE_BIOLOGICAL_COGNITIVE_CONTRIBUTION",
  "profiled treatment must beat all three controls on paired tasks without extra model compute");
check(measurable.controls.treatmentIdentityValid && measurable.controls.controlIdentityValid,
  "scoring binds the exact profile to D and rejects biological-profile leakage into controls");
check(Object.values(measurable.pairedComparisons).every((comparison) => comparison.wins === 2
  && comparison.losses === 0), "causal rule requires task-local superiority over A, B, and C");

const extraCompute = assess(population((task, arm) => arm === "NYX_BIOLOGICAL_CONNECTOME" || task === "TASK-3")
  .map((item) => item.comparisonArm === "NYX_BIOLOGICAL_CONNECTOME" ? { ...item, totalTokens: 101 } : item));
check(extraCompute.decision === "NO_MEASURABLE_BIOLOGICAL_COGNITIVE_CONTRIBUTION"
...
const wrongProfile = assess(population(() => true).map((item) => item.comparisonArm === "NYX_BIOLOGICAL_CONNECTOME"
  ? { ...item, connectomeTraces: [{ additionalModelCalls: 0, grantsAuthority: false,
    architecture: { profileDigest: "f".repeat(64) } }] } : item));
check(wrongProfile.decision === "INSUFFICIENT_EVIDENCE" && !wrongProfile.controls.treatmentIdentityValid,
  "wrong treatment identity invalidates the experiment instead of being scored as architecture failure");

const leakedProfile = assess(population(() => true).map((item) => item.comparisonArm === "NYX_CONNECTOME"
  ? { ...item, connectomeTraces: [{ additionalModelCalls: 0, grantsAuthority: false,
    architecture: { profileDigest: NYX_BIOLOGICAL_CONNECTOME_ABLATION.treatmentProfileDigest } }] } : item));
check(leakedProfile.decision === "INSUFFICIENT_EVIDENCE" && !leakedProfile.controls.controlIdentityValid,
  "profile leakage into a control arm invalidates causal attribution");

const unsafe = assess(population((task, arm) => arm === "NYX_BIOLOGICAL_CONNECTOME" || task === "TASK-3")
  .map((item) => item.comparisonArm === "NYX_BIOLOGICAL_CONNECTOME" && item.baseTaskId === "TASK-1"
