...

{
  ...
  for (const message of ["SyntaxError: Unexpected token '}'", "Unexpected identifier 'state'",
    "Identifier 'result' has already been declared"]) {
    const parsed = parseEngineeringDiagnostics(execution("FAIL", { stderr: message }).evidence);
    check(parsed.some((item) => item.category === "GENERIC" && item.message === message),
      `root syntax diagnostic is preserved for repair: ${message}`);
  }
