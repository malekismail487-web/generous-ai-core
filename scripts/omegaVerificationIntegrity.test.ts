...
import { NYX_ENGINEERING_QUALITY_V5, NYX_V5_FROZEN_CORE,
  NYX_V5_QUALITY_REPAIR_FROZEN_CORE, NYX_V5_QUALITY_REPAIR_V2_FROZEN_CORE,
  NYX_V5_QUALITY_REPAIR_V3_FROZEN_CORE } from "./omega/nyx-quality-v5-fixtures";
...
`;
  ...
  check(NYX_ENGINEERING_QUALITY_V4.length === 7 && new Set(NYX_ENGINEERING_QUALITY_V4.map((task) => task.taskClass)).size === 7,
    "V4 freezes seven fresh and distinct engineering task classes");
  check(NYX_ENGINEERING_QUALITY_V4.every((task) => task.provenance === "NYX_ENGINEERING_QUALITY_FRESH_HOLDOUT_V4"
    && task.initiallyAdmittedPaths.every((path) => !path.startsWith("tools/"))
    && task.availableEvidence.every((item) => !item.relativePath.startsWith("tools/"))),
  ...
  for (const task of NYX_ENGINEERING_QUALITY_V4) {
    v4TaskDigests.add(sha256(canonical(task)));
    const taskRoot = join(parent, "v4-ground-truth", task.taskId);
    const correctRoot = join(taskRoot, "candidate");
    const faultyRoot = join(taskRoot, "faulty");
    const hiddenTaskRoot = join(taskRoot, "hidden-evaluator");
    for (const [path, content] of Object.entries(task.correctFiles)) {
    ...
    const correctVisible = spawnSync(process.execPath, [join(correctRoot, "tools", "verify-visible.mjs")], { cwd: correctRoot });
    const faultyVisible = spawnSync(process.execPath, [join(faultyRoot, "tools", "verify-visible.mjs")], { cwd: faultyRoot });
    const hiddenEvaluator = await R3IsolatedHiddenEvaluator.create({ evaluatorId: `V4-GROUND-${task.taskId}`,
      evaluatorVersion: "nyx-quality-v4/1", candidateRoot: correctRoot, hiddenEvaluatorRoot: hiddenTaskRoot,
      hiddenCaseFile: "private/cases.json", expectedHiddenCaseFileSha256: sha256(hiddenTaskBytes),
      candidateRunner: "tools/candidate-runner.mjs", expectedCandidateRunnerSha256: sha256(OMEGA_CANDIDATE_RUNNER_SOURCE),
      candidateModule: task.candidateModule, exportName: task.exportName, timeoutMsPerCase: 3_000,
      maxOutputBytesPerCase: 8_192, maxCases: 32 });
    const hidden = await hiddenEvaluator.evaluate();
    const quality = assessEngineeringQuality({ assessmentId: `V4-GROUND-QUALITY-${task.taskId}`,
  ...
  check(Object.entries(NYX_V5_QUALITY_REPAIR_V3_FROZEN_CORE.files).every(([path, digest]) => {
    const blob = frozenGitBlob(NYX_V5_QUALITY_REPAIR_V3_FROZEN_CORE.commit, path);
    return blob !== null && sha256(blob) === digest;
  }), "measured-feedback successor is pinned without rewriting earlier quality-repair epochs");
