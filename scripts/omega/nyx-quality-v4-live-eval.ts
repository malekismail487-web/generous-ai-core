...
if (!["V4", "V5", "V5_QUALITY_REPAIR", "CHALLENGE", "FRONTIER_CHALLENGE", "CONTEXT", "CONTEXT_REPAIR", "CONTEXT_LINES", "COMPARISON", "CAPACITY_STATUS", "CONNECTOME_ABLATION", "BIOLOGICAL_CONNECTOME_ABLATION", "CONTRASTIVE_CIRCUIT_ABLATION"].includes(SUITE_ID)) throw new Error("unsupported_nyx_quality_suite");
...
const IS_ANY_CONNECTOME_ABLATION = IS_CONNECTOME_ABLATION || IS_BIOLOGICAL_CONNECTOME_ABLATION
  || IS_CONTRASTIVE_CIRCUIT_ABLATION;
...
const SOURCE_REPRESENTATION = IS_CONTEXT_LINES || IS_COMPARISON || IS_FRONTIER_CHALLENGE
  ...
  || IS_BIOLOGICAL_CONNECTOME_ABLATION || IS_CONTRASTIVE_CIRCUIT_ABLATION ? "LINES" : "TEXT";
...
const HOLDOUT: readonly EvaluationTask[] = IS_CONTRASTIVE_CIRCUIT_ABLATION
  ? NYX_ENGINEERING_QUALITY_V4.flatMap((task) => NYX_CONTRASTIVE_CIRCUIT_ABLATION.arms.map((comparisonArm) => ({
...
if (IS_CONTRASTIVE_CIRCUIT_ABLATION
...
function cognitionVariantFor(task: EvaluationTask): NyxCognitionExperimentVariant {
  ...
  if (task.comparisonArm === "NYX_REASONING_STACK" || task.comparisonArm === "NYX_CONNECTOME"
    ...
    || task.comparisonArm === "NYX_CONTRASTIVE_CIRCUIT") return "REASONING_ENABLED";
...
if (taskResults.length === HOLDOUT.length || IS_CONTEXT && taskResults.length > 0) {
  ...
  const result = { schemaVersion: 1, chunkId: `OMEGA-NYX-QUALITY-${SUITE_ID}-001`, suiteIdentity: SUITE_ID,
    ...
    sourceRepresentation: SOURCE_REPRESENTATION,
    intentCompilationMode: INTENT_COMPILATION_MODE,
    experimentVariant: IS_CONTRASTIVE_CIRCUIT_ABLATION ? "MATCHED_CONTRASTIVE_CIRCUIT_ABLATION"
  ...
  const contrastiveCircuitAssessment = IS_CONTRASTIVE_CIRCUIT_ABLATION
  ...
  const publishedReport = IS_CONTRASTIVE_CIRCUIT_ABLATION ? { ...result,
    chunkId: NYX_CONTRASTIVE_CIRCUIT_ABLATION.chunkId,
