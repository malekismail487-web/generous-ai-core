...
assert(records.every((item) => ["WRITE_SANDBOX", "PROVISION_SANDBOX", "TERMINATE_SANDBOX", "WRITE_SANDBOX_CONTENT"].every((authority) => item.authority.unavailable.includes(authority))), "all sandbox lifecycle and content mutation authorities remain unavailable");
