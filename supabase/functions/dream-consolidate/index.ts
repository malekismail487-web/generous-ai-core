...
serve(async (req) => {
  ...
  // ---- Cron mode: batch-process active students ----
  if (headerSecret && cronSecret && headerSecret === cronSecret) {
    const { data: students } = await supa.from("profiles")
      .select("id, school_id")
      .eq("user_type", "student").eq("is_active", true).eq("status", "approved")
      .limit(2000);
    const list = students ?? [];
    ...
    for (const s of list) {
      try {
        const r = await generateForUser(supa, s.id, s.school_id, apiKey, today);
        if (r.skipped || r.cached) skipped++;
        else if (r.error) failed++;
        else ok++;
      } catch { failed++; }
      await new Promise(res => setTimeout(res, 250)); // gentle pacing
    }
    return new Response(JSON.stringify({ mode: "cron", processed: list.length, ok, skipped, failed }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }

  // ---- Single-user mode (called from dashboard) ----
  const user = await getUser(req);
  if (!user) {
    return new Response(JSON.stringify({ error: "unauthorized" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  }
  const result = await generateForUser(supa, user.id, user.schoolId, apiKey, today);
