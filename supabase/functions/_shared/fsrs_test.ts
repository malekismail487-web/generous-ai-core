...
Deno.test("FSRS: retrievability decays monotonically with elapsed time", () => {
  const c = fsrsUpdate(newFsrsCard(), 3, 0);
  const r0 = fsrsRetrievability(c, 0);
  const r1 = fsrsRetrievability(c, 1 * DAY);
  const r7 = fsrsRetrievability(c, 7 * DAY);
  const r30 = fsrsRetrievability(c, 30 * DAY);
  assert(r0 > r1 && r1 > r7 && r7 > r30, `expected decay, got ${r0},${r1},${r7},${r30}`);
});

Deno.test("FSRS: lapse reduces stability and increments lapses", () => {
  let c = fsrsUpdate(newFsrsCard(), 3, 0);
  const sBefore = c.S;
  c = fsrsUpdate(c, 1, 7 * DAY);
