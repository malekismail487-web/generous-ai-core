...
DECLARE
  profile_ids uuid[];
BEGIN
  -- Collect profile IDs first
  SELECT array_agg(id) INTO profile_ids FROM public.profiles WHERE school_id = school_uuid;
  ...
  -- Delete user_roles for profiles in this school
  IF profile_ids IS NOT NULL THEN
    DELETE FROM public.user_roles WHERE user_id = ANY(profile_ids);
  END IF;
