  ...
  return (
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border bg-foreground/[0.04] backdrop-blur-2xl p-8 text-center text-sm text-muted-foreground">
          No changes recorded yet.
        </div>
      ) : (
        <ol className="space-y-2">
          {versions.map(v => (
            <li key={v.id} className="rounded-lg border border-border bg-foreground/[0.04] backdrop-blur-2xl p-3 text-sm">
