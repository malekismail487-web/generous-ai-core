...
export default function SuperAdmin() {
  ...
  if (testingRole !== 'none') {
    ...
    // For student testing, show the actual student app experience.
          ...
          <div className="fixed top-0 left-0 right-0 z-[100] bg-amber-500 text-amber-950">
            <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-sm">🧪 Testing as Student</span>
              </div>
              <Button
                size="sm"
                variant="outline"
    ...
    // Teacher and School Admin previews render live, read-only data for a
    // chosen school — never mock records.
    return (
      <RolePreviewPanel
        role={testingRole as 'teacher' | 'school_admin'}
        schools={schools.map((s) => ({ id: s.id, name: s.name }))}
        onExit={() => setTestingRole('none')}
      />
    );
  }
